...
On top of that comes that I regard it more useful to stay close to core technologies, and not get distracted by abstractions, abstractions like preprocessors. In the short run we may benefit from knowing how to work with abstractions; but in the long run we get much greater rewards from knowing the underlying technologies. And therefore we’ll focus on standard CSS rather than non-standard Sass, Less, Stylus, and whatnot.
